...
class ProductApiController with ApiHelper {
  ...
  Future<List<Product>> getStoreProducts({required int storeId}) async {
    ...
    if (response.statusCode == 200) {
      ...
      var storeProductsList = productJsonList
          ...
          .where((productData) => productData.id == storeId)
          .toList();
  ...
  Future<bool> createProduct({required Product product}) async {
    ...
    for (int i = 0; i < product.productImages!.length; i++) {
      var imageFile = await http.MultipartFile.fromPath(
          'image$i', product.productImages![i].imageUrl);
      request.files.add(imageFile);
    }
    request.fields['store_id'] = product.store.id.toString();
    request.fields['describtion'] = product.name;
    // request.fields['quantity'] = product.quantity.toString(); // يجب اضافة خاصية quantit للبرودكت
    request.fields['name'] = product.name;
    request.fields['category_id'] = '10';
    ...
    if (response.statusCode == 200) {
      ...
      Get.snackbar('فشلت الإضافة', 'حاول مجدداً', colorText: kPrimaryColor);
      return false;
    }
  }
  ...
  Future<bool> updateProduct({required Product product}) async {
    ...
    if (response.statusCode == 201) {
      Get.snackbar('نجح التحديث', 'تم إضافة المنتج بنجاح',
          colorText: kPrimaryColor);
      return true;
    } else {
      Get.snackbar('فشل التحديث', 'حاول مجدداً', colorText: kPrimaryColor);
      return false;
    }
  }
  ...
  Future<bool> saveRating({required double rating, required int productId}) async {
      var url = Uri.parse(ApiSettings.rateProduct.replaceFirst('/{id}', ''));
      final response = await http.post(
        url,
        body: {
          'product_id': productId.toString(), // Replace with the product's ID
      ...
      if (response.statusCode == 200) {
        Get.snackbar('شكراً لك', 'سيتم أخذ تقييمك بالاعتبار',colorText: Colors.black);
        return true;
      } else {
  ...
  Future<double> showRating({required int productId}) async {
    ...
    if (response.statusCode == 200) {
      ...
      double productRating = responseData['data']['data'][0]['rating'].toDouble();
